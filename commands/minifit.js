...
module.exports = {
    ...
    callback: async function (arg) {
        ...
        if (ratelimits.has(message.author.id)) {
            return sendErrorMessage(["أنت تقوم بتصغير سكربت حاليًا. الرجاء الانتظار!", "خطأ", "تحديد المعدل"], message)
        }

        const process_embed = createEmbed({
            fields: [
                { name: `🔄 المعالجة`, value: `${getEmoji("loading")} يتم إنشاء الجلسة...` }
            ],
            timestamp: true,
            color: Colors.Yellow,
            footer: {
